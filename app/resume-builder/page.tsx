  ...
  return (
    <Provider store={store}>
      ...
      <div className="flex h-full overflow-hidden bg-slate-50 dark:bg-slate-950 relative">
        ...
        <div className={`w-full lg:w-1/2 h-full overflow-y-auto border-r border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900 ${activeTab === "edit" ? "block" : "hidden lg:block"}`}>
          <ResumeForm />
        </div>

        {/* RIGHT — Fixed resume preview panel, never scrolls */}
        <div className={`w-full lg:w-1/2 h-full overflow-hidden bg-slate-100 dark:bg-slate-950 flex flex-col ${activeTab === "preview" ? "flex" : "hidden lg:flex"}`}>
          <Resume />
        </div>
